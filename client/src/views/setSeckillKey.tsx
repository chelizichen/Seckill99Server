  ...
  const onFinish = async (values) => {
    try {
      ...
      if (response.data.error) {
        alert('Failed to create seckill activity: ' + response.data.error);
      } else {
        alert('Seckill activity created successfully!');
        nag('/seckill-list');
      }
    } catch (error) {
      alert('Failed to create seckill activity: ' + error.message);
    }
  };
    ...
    <div>
      ...
      >
        ...
        <Form.Item
          label="Seckill Count"
          name="seckillCount"
          rules={[{ required: true, message: 'Please enter inventory amount!' }]}
        >
